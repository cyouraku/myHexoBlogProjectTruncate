...
public interface WeatherWeatherService {
	...
	/**
	 * 通过城镇名字查询Today天气List<WeatherWeatherDTO>，RESTful接口
	 * @param stationName
	 * @return
	 */
	@GET
	@Path("/getWeatherByStationNameList")
	@Produces({MediaType.APPLICATION_JSON + ";charset=UTF-8"})
	public List<StationDetailWeatherDTO> selectByStationNameRestDto();

	/**
	 * 通过城镇名字查询Next6day天气List<WeatherWeatherDTO>，RESTful接口
	 * @param stationName
	 * @return
	 */
	@GET
	@Path("/getWeatherByStationNameList2")
	@Produces({MediaType.APPLICATION_JSON + ";charset=UTF-8"})
	public List<StationDetailWeatherDTO> selectByStationNameRestDto2(@QueryParam("stationName") String stationName);

	/**
	 * 通过城镇名字查询Hour天气List<WeatherWeatherDTO>，RESTful接口
	 * @param stationName
	 * @return
	 */
	@GET
	@Path("/getWeatherByStationNameList3")
	@Produces({MediaType.APPLICATION_JSON + ";charset=UTF-8"})
	public List<StationDetailWeatherDTO> selectByStationNameRestDto3(@QueryParam("stationName") String stationName);
}
